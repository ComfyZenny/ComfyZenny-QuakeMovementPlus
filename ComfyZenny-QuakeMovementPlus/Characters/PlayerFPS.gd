...
export var x_mouse_sensitivity = .04
export var y_mouse_sensitivity = .04
...
export var side_strafe_acceleration = 10
...
export var air_control_acceleration = 200
export var dash_speed_ground = 200
export var dash_speed_air = 50
...
onready var assault_cannon = $Head/Camera/AssaultCannon
...
func weapon_select():
	...
	if Input.is_action_just_pressed("AssaultCannon"):
		current_weapon = 1
	if Input.is_action_just_pressed("Shotgun"):
		current_weapon = 2
		
	if current_weapon == 1:
		assault_cannon.visible = true
		assault_cannon.fire()
		current_ammo = assault_cannon.current_ammo
		clip_size = assault_cannon.clip_size
		reloading = assault_cannon.reloading
	else:
		assault_cannon.visible = false
	if current_weapon == 2:
		shotgun.visible = true
		shotgun.fire()
		current_ammo = shotgun.current_ammo
		clip_size = shotgun.clip_size
		reloading = shotgun.reloading
	else:
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ability") and player_abilities.crouchslide:
		if touching_ground and not is_crouchsliding:
			is_crouching = true
		elif not head_hit:
			is_crouching = false
	...
	player_velocity = move_and_slide(player_velocity, up, true)
	touching_ground = foot.is_colliding()
...
func _input(ev):
	if (ev is InputEventMouseMotion):
		...
		head.rotate_x(-deg2rad(ev.relative.y) * y_mouse_sensitivity)
